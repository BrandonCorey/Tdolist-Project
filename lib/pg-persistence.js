...
class PgPersistence {
  ...
  async createUserAccount(username, password) {
    const CREATE_ACCOUNT = `INSERT INTO users
                              (username, password)
                              VALUES
                                ($1, $2)`;
    ...
    try {
      ...
      let result = await dbQuery(CREATE_ACCOUNT, username, hashedPass);
      return result.rowCount > 0;
    } catch (error) {
      if (this.isUniqueConstraintViolation(error)) return false
      throw error;
  ...
  async existsUsername(username) {
    const ALL_USERNAMES = `SELECT *
                           FROM users
                           WHERE username = $1`;

    let result = await dbQuery(ALL_USERNAMES, username);
  ...
  // Function designed to handle race conditions if multiple sessions open
